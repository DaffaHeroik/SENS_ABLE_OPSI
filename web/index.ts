...
import { registerFirmwareRoutes } from "../firmwareRoutes";
...
async function startServer() {
  ...
  // The public ESP32 gateway receives compact JSON only.
  ...
  app.use((error: unknown, _request: express.Request, response: express.Response, next: express.NextFunction) => {
    if (error instanceof SyntaxError && "body" in error) {
      return response.status(400).json({ accepted: false, code: "invalid_json" });
    }
    return next(error);
  });
