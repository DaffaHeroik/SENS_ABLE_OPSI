...
String buildHTML(String msg = "") {
  ...
  if (dataReady) {
    ...
    html += "<div class='card'><h2>Hasil Terakhir &mdash; " + respName + " &nbsp;";
    ...
    html += "<div class='g2'>";
    html += "<div class='sb'><div class='sv'>" + respAge + " th &nbsp;|&nbsp;" + respGender + "</div><div class='sl'>Usia &amp; Gender</div></div>";
...
void handleRecord() {
  ...
  if (respName == "" || glucoseRef <= 0 || respBerat <= 0 || respTinggi <= 0) {
  ...
  while ((millis() - recStart) < (RECORD_SEC * 1000UL)) {
    sensor.update();
    uint16_t ir, red;
    while (sensor.getRawValues(&ir, &red) && sampleCount < MAX_SAMPLES - 1) {
  ...
  String okMsg = "<div class='msg msg-ok'>&#10003; Data <b>" + respName + "</b> berhasil direkam! (" + String(sampleCount) + " samples)</div>";
...
void setup() {
  ...
  if (!LittleFS.begin(true)) { // 'true' artinya format memori jika gagal mount
